...
def run(phone, i):
    ...
    # ✅ Ghi nhận trạng thái spam
    if phone not in spam_tracker:
        spam_tracker[phone] = {"current": 0, "total": 0, "done": False}

    spam_tracker[phone]["current"] = i
    spam_tracker[phone]["total"] += 1  # hoặc đặt cố định nếu muốn
    spam_tracker[phone]["done"] = False
...
def spam():
    ...
    try:
        ...
        return jsonify({"error": "Số lần spam phải là số nguyên"}), 400

    if not phone or not count:
        return jsonify({"error": "Vui lòng cung cấp số điện thoại và số lần spam"}), 400

    result = [f"Đang bắt đầu spam cho số điện thoại {phone}"]
    for i in range(1, count + 1):
        result.append(run(phone, i))
        result.append(f"Spam thành công lần: {i}")
        for j in range(4, 0, -1):
            result.append(f"Vui lòng chờ {j} giây...")
            time.sleep(1)

    return jsonify({"message": "Spam hoàn tất", "results": result})

@app.route('/status', methods=['GET'])
def status():
    phone = request.args.get('phone')
    if not phone:
        return Response(json.dumps({"error": "Thiếu số điện thoại"}, ensure_ascii=False), content_type='application/json'), 400

    info = spam_tracker.get(phone)
    if not info:
        return Response(json.dumps({"status": "Chưa spam hoặc đã xóa khỏi bộ nhớ"}, ensure_ascii=False), content_type='application/json'), 404
